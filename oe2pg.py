    ...
    def execute_with_cursor(self, operation_func, description=""):
        ...
        try:
            ...
            if description.startswith("get_") or description.startswith("count_") or description.startswith("fetch_"):
                ...
                if table_name and ("permission denied" in str(e).lower() or "access denied" in str(e).lower()):

                    self.logger.log(f"Authorization error detected for {table_name}. Adding to ignore list.")
        ...
        try:
            ...
            if not self.postgres.table_exists(pg_table_name):
                ...
                if not self.postgres.create_table(pg_table_name, columns):
                    ...
                    self.logger.log(f"Failed to create table {pg_table_name} in PostgreSQL")
                    return False
            ...
            if truncate:
                ...
                if not self.postgres.truncate_table(pg_table_name):
                    ...
                    self.logger.log(f"Failed to truncate table {pg_table_name}")
                    return False
            ...
            if "permission denied" in str(e).lower() or "access denied" in str(e).lower():
                ...
                self.logger.log(f"Authorization error detected for {schema}.{table_name}. Adding to ignore list.")
